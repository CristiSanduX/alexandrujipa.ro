...
const makeFadeUp = (loaderOffset: number): Variants => ({
  hidden: { opacity: 0, y: 30 },
  visible: (delay: number) => ({
    opacity: 1,
    y: 0,
    transition: { duration: 0.8, ease: EASE, delay: loaderOffset + delay },
  }),
});

export default function Hero() {
  const loaderOffset = typeof window !== "undefined" && sessionStorage.getItem("loader_seen") ? 0 : 1.8;
  const fadeUp = makeFadeUp(loaderOffset);
  ...
  return (
    <section data-navbar-theme="dark" className="relative h-[100dvh] min-h-[600px] overflow-hidden flex items-center justify-center">
      ...
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        transition={{ delay: loaderOffset + 1.2, duration: 0.8 }}
      ...
      >
        ...
        >
          <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
            <path d="M10 3 L10 17 M4 11 L10 17 L16 11" stroke="white" strokeOpacity="0.5" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
        </motion.div>
      </motion.div>
    </section>
